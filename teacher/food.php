...
<div class="modal" id="add-food">
  ...
  <form class="report-card-form" action="add_food.php" method="POST">
  <h2 style="text-align:center;">Adding New Food to Menu</h2>
  <br><br>
  <label for="student_name">Food Name:</label>
  <input type="text" name="student_name" id="student_name" required>
  
  <br><br>
    <label for="subject">Food Ingredients:</label>
...
echo '<button class="add-button" onclick="showFood()">Add new Food to the Menu</button>';
...
if (mysqli_num_rows($result2) > 0) {
  ...
  echo '<h2 class="container-title">Students with Food Allergy</h2>';
    while ($row = mysqli_fetch_assoc($result2)) {
      $foodAllergy = $row['ID'];
      $AStudentName = $row['student_name'];
      $AfoodName = $row['food_name'];
      $AfoodContent = $row['Ingredient'];
      $AfoodComment = $row['comment'];
        
        
        
        echo '<div class="allergy-card">';
        echo '<input style="display:none;" value=" ' . $foodAllergy .'">';
        echo '<h3>Student Name : ' . $AStudentName . '</h3>';
        echo '<h3>Food : ' . $AfoodName . '</h3>';
        echo '<h4>' . $AfoodContent . '</h4>';
        echo '<h4>' . $AfoodComment . '</h4>';
        echo '</div>';
    ...
    echo "No one in the school has allergies.";
  }
  echo '</div>';
echo '</div>';
?>
