...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    if (mysqli_num_rows($resultDetails) > 0) {
        ...
        echo "<input type='text' style='display:none' name='studentname' value='$selectedStudentId' class='form-input' readonly><br>";
        echo "<input type='text' id='student-name' name='student' value='$studentName' class='form-input' readonly><br>";

        echo "<label for='pending-payment' class='form-label'>Pending Payment:</label>";
        echo "<input type='text' id='pending-payment' name='pendingpayment' value='$pendingPayment' class='form-input' readonly><br>";

        echo "<label for='payment-status' class='form-label'>Payment Status:</label>";
        echo "<input type='text' id='payment-status' name='paymentstatus' value='$paymentStatus' class='form-input' readonly><br>";

        echo "<label for='amount' class='form-label'>Amount:</label>";
        echo "<input type='text' id='amount' name='amount' value='$amount' class='form-input' readonly><br>";

        echo "<input type='hidden' name='student-id' value='$selectedStudentId'>";
        echo "<input type='submit' value='Update Payment' class='select-btn submit-button'>";
        echo "</form>";
    } else {
